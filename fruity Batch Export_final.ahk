...
Export() ; remove this line to get the normal dialog back
return
...
Export()
{
	...
	;if Directory =
	;	Exit
    Directory := "C:\Crea\Audio files" ; remove this line to get the normal dialog back

	;User selects folder containing FLP files
	;FileSelectFolder, ExportDirectory, , 0, Select directory to save Mp3s.
	;if ExportDirectory =
	;	Exit
    ExportDirectory := "C:\Users\TK\Desktop\FL exported all" ; remove this line to get the normal dialog back
	...
	Loop %FileCount%
	{
        ...
        OutputFile = %ExportDirectory%\%Name_no_ext%.mp3
        ;#ErrorStdOut, %OutputFile%
        IfExist, %OutputFile%
        {
            continue
        }
        ...
        try
        {
            ...
            Warning := true
            count := 0
            while Warning and count < 5
            {
                Warning = false
                SetTitleMatchMode, 2 ; Partially match with window title name "Rendering to"
                IF WinExists, Warning ; Dont find missing data files
                {
                    ControlSend, , {n}, Warning
                    Warning = true
                    Sleep, 1000
                    ControlSend, , {Enter}, Warning ; Don't find missing VSTs
                    Warning = true
                }
                IF WinExists, Debug ; Dont find erroneous VSTs
                {
                    ControlSend, , {Enter}, Debug
                    Warning = true
                }
                IF WinExists, Error ; Dont find missing data files
                {
            ...
            Sleep 1000 ; Wait to see if the "Confirm Save As" box comes up
            ...
            ELSE ; Continue exporting the song
            {
                ...
                Sleep, 5000 ; Make sure the song starts exporting before checking if it's done
		...
		try
        {
            Sleep 1000 ; Wait for extra secure stuffs
            SetTitleMatchMode, 2 ; Partially match with window title name "[flp] FL Studio 12"
            WinClose, FL Studio
            SetTitleMatchMode, 2 ; Partially match with window title name "Rendering to"
            IF WinExists, Confirm ; Dont save when quitting
            {
                ControlSend, , {n}, Confirm
            }
        }
		Sleep 2000 ; Wait for extra secure stuffs
	}
